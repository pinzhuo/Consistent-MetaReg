...
def main():
    ...
    for epoch in range(1, args.epoch+1):
        ...
        train_dataset = Office31Dataset(num_classes=args.n_way, num_support=args.k_spt,
                                      num_query=args.k_qry, num_epoch=1000, phase='train')
        ...
        for i, batch in enumerate(tqdm(dloader_train)):
            ...
            accs, loss_cls, loss_f = maml.forward(data_support, labels_support, data_query, labels_query)

            train_accuracies.append(accs)

            if i % 100 == 0:
                avg_accs = np.array(train_accuracies).mean(axis=0).astype(np.float16)

                print(
                    'Train Epoch: {} \tLoss_cls: {:.4f} \tLoss_f: {:.4f}'.format(
                        i, loss_cls, loss_f), '\ttraining acc:', avg_accs )

                #print('step:', i, '\ttraining acc:', '\t loss cls .4f:', avg_accs)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--save_path', type=str, help='model save path', default='/data/liuyong/TianPinzhuo/IJCAI/reverse_MAML_office31_seed')
    argparser.add_argument('--trade_of', type=float, help='trade of',
                           default='1')
