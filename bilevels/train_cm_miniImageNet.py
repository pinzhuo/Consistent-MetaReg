...
from models.cm_head import CM_R2d2Head, CM_SVMHead
...
def get_model(opt):
    ...
    if opt.cls_head == 'R2D2':
        cls_head = CM_R2d2Head().cuda()
    elif opt.cls_head == 'SVM':
        cls_head = CM_SVMHead().cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cls-head', type=str, default='SVM',
                        help='R2D2 or SVM')
    ...
    parser.add_argument('--save-path', default='/data/liuyong/TianPinzhuo/IJCAI/reverse_SVM_tieredmageNet_5way_batch4')
    trade = 5
    ...
    dloader_val = data_loader(
        ...
        nTestNovel=opt.val_query * opt.train_way, # num test examples for all the novel categories
        nTestBase=0, # num test examples for all the base categories
        batch_size=1,
        num_workers=0,
...
#---------------------------------------------------
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dloader_train(epoch)), 1):
            ...
            if opt.cls_head == 'SVM':
                reverse_loss = cls_head.CMloss_Fnorm(emb_support, emb_query, labels_query, opt.train_way, opt.train_query)
            else:
                reverse_loss = cls_head.CMloss_Fnorm( labels_query, opt.train_way,
                                                     opt.train_query)
            ...
            if (i % 100 == 0):
                ...
                log(log_file_path,
                    'Train Epoch: {}\tBatch: [{}/{}]\tEntroy Loss: {:.4f}\tReverse Loss: {:.4f}\tAccuracy: {:.2f} % ({:.2f} %)'.format(
                        epoch, i, len(dloader_train), loss.item(), reverse_loss.item(), train_acc_avg, acc))

            optimizer.zero_grad()
            total_loss = loss + trade*reverse_loss
            total_loss.backward()
            optimizer.step()
